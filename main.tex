...
\title{SPCF: Interpreter Development and Affine Program Transformation}
\author{Jay Rabjohns}
...
probably the problem I'm trying to solve and all of that. Maybe look at first thing I wrote

It is common practice in functional languages to nest function calls, that is, to share variables from a function's scope as arguments of another function. It allows for incredibly terse and expressive programs but complicates their implementation. It is theoretically possible to refactor programs to an equivalent form while eliminating this behaviour \cite{laird_2007}. More formally, any term of the SPCF is equivalent to another term which satisfies affine typing rules, which we will discuss later on. This is what forms the basis of the project, where we will endeavour to implement an interpreter for SPCF capable of performing this refactoring action.

aliken it to refactoring tail recursion etc. 

probably cover a bit about interpreter, semantics, based on lambda calculus and journey up.
...
\begin{enumerate}
    \item Functions are uncurried, meaning that terms of a product type are treated as a single argument.
    \item Terms must be closed. 
\end{enumerate}

Terms must be closed because catch does not keep track of the state of other parts of the program. It would be theoretically possible to implement catch without this requirement by having a global control stack which kept track of all binding variables then by offsetting the index returned by catch with the legnth of the global stack. However this would have non-obvious implications on the index of unbound variables which would be negative.

This method assumes that functions are uncurried, meaning that the product types are treated as a single argument. This relies on functions being uncurried function representation If an error is evaluated in this way, the current length of the control stack is returned, indicating which 
...
The interpreter is complex and relies on many typing and evaluation rules, creating many opportunities for bugs to shroud themselves in non obvious ways. This is exasperated by the prevelance of continuations and callbacks in the the program logic of the denesting action, implemented in Chapter \ref{chapter: denesting}, leading to cryptic and misleading errors being thrown far away from where the bug lies. This creates the perfect conditions for a kind of 'second-order' logical error, where it is hard discerning whether issues relate to the internals of the interpreter or the program logic built on top of it.

In lieu of a proper debugger for SPCF programs, the interpreter is able to output logs of intermediate evaluation and type inference steps, approximating a kind of call stack for term manipulations. These logs may be observed and compared against solutions written by hand.

Unit tests help verify that individual components are working as expected, narrowing the potential scope for bugs, or at least explicitly stating erroneous logic as intentional at the time. The project heavily relies on a combination of unit tests and by hand evaluation to verify that its behaviour is correct. One of the primary benefits of units tests is that they may be algorithmically regressed upon as more features are added, which is enforced by a GitHub action acting as a simple CI system \footnote{\url{https://github.com/jayrabjohns/dissertation-refactoring-spcf/blob/main/.github/workflows/haskell.yml}}.
...
It doesn't make sense that projection is 1 indexed sometimes nad 0 index others.

I think the I and j and the wrong way around in injection. consequently I should be in the lhs of the binary product.
...
\begin{definition}\label{def:proj}
    ...
    \[
    \lambda tx. \text{case } \langle \pi_1(t), \langle \text{case } \langle \pi_i(x), \langle \pi_j(\pi_2(t)) \lceil \langle \pi_0(x), \dots, \pi_m(x)\rangle\rceil_j\ | \ j < n \rangle \rangle\ |\ i \leq m \rangle \rangle
    \]
\end{definition}
...
\begin{enumerate}
    \item Care is needed when defining if function arguments and tuples are 0-indexed or not. Both terms rely on the definition of catch to index a tuple, so their indexing must be consistent. This is another justification for using a stateless catch operation as mentioned in Section \ref{section: catch}.
...
\begin{definition}
    ...
    In other words, elements from $X$ may be recovered from $Y$ after an initial mapping without information loss. It is in some senses a weaker form of bijection, where there is no implication of f and g being invertable.
\end{definition}

The definitions for proj and inj form an affinely definable retraction 
\begin{equation}
    \underline{n}^m \Rightarrow \underline{0} \trianglelefteq \underline{m}^{n^m \cdot (m + 1)} \quad (\forall n > 0)
\end{equation}

It is true that terms of any type in the SPCF may be affinely typeable. 
There is another retract to transform types into the form readily usable by the described transformation. 
That other retract is : 


Inj and proj form an inductive base case, after transforming terms in the other way, they may have this applied to them.

Inj effectively reduces the order of a type by one. 
this step may be repeated $m+1$ times to get a term of the type:
\begin{align}
...
\end{align}
Each step reduces the order of the terms type by one down to 0. Regard that the inner most product of continuations in the $m+1$ case has elements of the type $n^0 \Rightarrow \underline{0} \equiv I \Rightarrow \underline{0}$ which is isomorphic to $\underline{0}$. Thus a function can be entirely encoded as a product.

The original type structure is recoverable with proj.
\begin{align}
...
The repeated application of these terms requires some nuance. Clearly, their output types for a single step do not match the corresponding input types. It must first pattern match on the terms to selectively apply the relevant step to the correct subterm. 

For inj, recognise that we must match on a sub term of the type  $\llbracket n^{m} \Rightarrow \underline{0} \rrbracket$. Considering the concrete definition for inj \eqref{def:inj} the relevant subterm to reapply to is each element of the $n$-fold product:
\[
...
Similarly for proj \eqref{def:proj}, we must find subterms with the type $\llbracket \underline{m+1} \times (n^m \Rightarrow \underline{0})^n \rrbracket$. In this case we must first traverse the term until reaching the previous base case:
\[\underline{1} \times(n^0 \Rightarrow \underline{0})^n\]
then work backwards by using the same pattern as for inj \eqref{eq:patten_match} to reapply proj to each result.

Repeatedly applying inj as a step requires some nuance as clearly the output type of a single application is not the same as the input type. 

Its original type structure is recoverable by proj. proj(inj(f)) = f.



other retract for types other than in the definition
...
recovering the type structure from repeated applications of inj. 
...
\chapter{Results}
The transformation applied to a term with type $o \rightarrow o \rightarrow \underline{0}$ can be seen in Appendix \ref{}. Clearly the term size grows very large.

We see no improvements to readability, but that could be due to the text formatting.

[graph of upper bound affecting runtime and term size]

[graph of type size affecting runtime and term size]

It quickly becomes unfeasible to apply this transformation to a function, let alone an entire codebase. That being said, the interpreter was built to favour correctness over speed, so it is possible that this could be mitigated with enough optimisation.
...
Major achievements:
interpreter for SPCF, a the time of writing there is no widely available interpreter for SPCF. A working parser and primitive type inference algorithm.

able to write programs in in its syntax and experiment with features.

Affinely definable denesting action for bounded terms

implement non-local control function catch, which is an uncommon feature for a lambda calculus.

Commented on the details of the denesting action, its purpose and insights on implementation.

Future work is possible to fully implement affinely typed denesting action for unbounded terms.

a natural progression is to implement denesting of unbounded terms, a large part of the complicated machinery is assembled already.
...
would be good to have a simpler example to step through. Then perhaps just define faactorial and the such as a last example and show it works.
