...
The pair of functions inj and proj form an affinely typeable retraction \eqref{eq:retraction_step}, proven by their definable terms \eqref{def:inj} and \eqref{def:proj}. A full proof can be found in the original literature, but informally there is quite explicitly a retraction from each type to one of a lower order given by Definitions \eqref{def:inj} and \eqref{def:proj}. By induction on $m$ it is possible to derive \eqref{eq:full_retraction}.
...
There is a flip-flop correspondence between the size of the term and the size of the type, to the point where it becomes feasibly impractical as a real-world transformation. From Table \ref{tab:affine-term-sizes}, to rewrite a function with 4 arguments ($m=3$) in a language with only boolean values ($n=2$) it would require roughly 50-70 thousand pages\footnote{Page count estimated from the character count using \url{https://charactercounter.com/characters-to-words}} to write out the transformed program using no linebreaks. 

It also requires significant computational resources, the entries marked $?$ in Tables \ref{tab:affine-term-sizes}, \ref{tab:timings} indicate that the machine running the transformation ran out of available memory after using $\sim$16GB with 2GB swap. The exponential increase in term size and execution time is further illustrated in Appendix \ref{appendix:term-growth}.
...
All testing was performed on a simple term of the form:
\[
\lambda x^{\underline{n}^{m+1}}. \text{case } \langle \pi_0(x), \bot^n \rangle : \llbracket \underline{n}^{m+1} \Rightarrow \underline{0} \llbracket 
\]
...
\begin{figure}[!ht]
    ...
    \caption{3D projection of log term size of inj($M$) against $n,m$.  }
...
\end{subfigure}
\caption{Term size of inj($M$)in relation to hyperparameters $n,m$. Note the log scale.}
